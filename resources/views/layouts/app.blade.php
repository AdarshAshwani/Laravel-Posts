...
<body class="h-full">
    ...
    <div class="min-h-screen flex">
        <!-- Sidebar -->
        <aside id="sidebar"
            class="hidden lg:flex lg:fixed lg:inset-y-0 lg:left-0 lg:w-68 lg:h-screen overflow-y-auto lg:flex-col shrink-0 glass surface">
            <div class="h-16 px-5 flex items-center gap-3">
                <a href="@auth {{ route('admin.dashboard') }} @else {{ route('posts.public') }} @endauth"
            ...
            <nav class="mt-3 px-3 space-y-1 text-sm">
                <a href="{{ route('admin.dashboard') }}"
                    ...
                    <svg class="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                        <path d="M3 12l9-9 9 9-1.5 1.5L12 5l-7.5 8.5L3 12zm2 8v-6h5v6H5zm7 0v-9h5v9h-5z" />
                    </svg>
                    <span class="font-semibold">Dashboard</span>
                </a>
                <a href="{{ route('posts.public') }}" class="nav-item">
                    <svg class="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
        ...
        <!-- Mobile drawer -->
        <aside id="drawer"
            class="fixed z-40 inset-y-0 left-0 w-72 surface transform -translate-x-full transition-transform duration-300 lg:hidden">
            {{-- keep your mobile drawer contents here --}}
        </aside>

        <!-- Main -->
        <div class="flex-1 min-w-0 flex flex-col lg:ml-68">
            <header
                class="sticky top-0 z-20 glass surface-strong border-b border-slate-200/70 dark:border-slate-800/70">
                <div class="h-16 px-4 sm:px-6 lg:px-8 flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-2">
                        {{-- SEARCH FORM (works on any page using url()->current()) --}}
                        <form action="{{ url()->current() }}" method="GET" class="hidden md:block" role="search"
                            aria-label="Search posts by content">
                            <div class="relative">
                                <input class="input w-72 pl-10" type="search" name="q" value="{{ request('q') }}"
                                    placeholder="Search posts by content" aria-label="Search posts" />
                                @if(request('q'))
                                <a href="{{ url()->current() }}"
                                    class="absolute right-2 top-1/2 -translate-y-1/2 text-slate-500 hover:underline text-sm">
                        ...
                        <div class="relative">
                            <button id="user-menu-btn"
                                ...
                                <img src="{{ asset('storage/'.auth()->user()->avatar_path) }}?v={{ optional(auth()->user()->updated_at)->timestamp }}"
                                    ...
                                    class="w-8 h-8 rounded-xl object-cover ring-2 ring-white/60 dark:ring-slate-900/60"
                                    loading="lazy" decoding="async">
                                @else
                                <div class="w-8 h-8 rounded-xl bg-gradient-to-br from-blue-400 to-blue-600 ring-2 ring-white/60 dark:ring-slate-900/60
           flex items-center justify-center text-white text-sm font-semibold">
                                    {{ strtoupper(substr(auth()->user()->username ?? auth()->user()->email, 0, 1)) }}
                                </div>
                                @endif
                                <span
                                    class="hidden sm:block text-sm font-semibold">{{ auth()->user()->username ?? auth()->user()->email }}</span>
                            ...
                            <div id="user-menu"
                                class="hidden absolute right-0 mt-2 w-48 rounded-xl surface shadow-lg overflow-hidden"
                                role="menu" aria-label="User Menu">
                                <a href="{{ route('profile.show') }}"
                                    class="block px-4 py-2 text-sm hover:bg-slate-50 dark:hover:bg-slate-800">Profile</a>
                                <div class="border-t border-slate-200/70 dark:border-slate-800/70"></div>
                                <form method="POST" action="{{ route('logout') }}"> @csrf
                                    <button
                                        class="block w-full text-left px-4 py-2 text-sm text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/20">Logout</button>
                                </form>
                            </div>
                        </div>
                        @endauth
                        ...
                        @guest
                        <a href="{{ route('login') }}"
                            class="text-sm font-semibold text-blue-600 hover:underline">Login</a>
                        @endguest
                    </div>
                </div>
            </header>
            ...
            @if(session('message'))
            <div id="toast" class="toast opacity-100 pointer-events-auto" role="status" aria-live="polite">
                <svg class="w-5 h-5" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                    <path d="M9 12l2 2 4-4 1.5 1.5L11 17l-4.5-4.5L8 11l1 1z" />
                </svg>
                <div id="toastText" class="text-sm font-semibold">{{ session('message') }}</div>
                <button class="ml-3 opacity-80 hover:opacity-100"
